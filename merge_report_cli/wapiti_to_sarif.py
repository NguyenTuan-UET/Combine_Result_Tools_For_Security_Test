...
# --- Hàm phụ trợ để xây dựng Taxonomy WSTG ---
def _build_wstg_taxonomy_entry(wstg_ids: set) -> dict:
    wstg_taxonomy = {
        # Đã bỏ "tool": "wapiti" khỏi đây để tuân thủ SARIF chuẩn hơn
...
# --- Hàm phụ trợ để xây dựng Taxonomy CWE ---
...
# --- Hàm chính để chuyển đổi Wapiti sang SARIF ---
def wapiti_to_sarif_converter(wapiti_json_data: dict) -> dict:
    ...
    for classification_name, details in classifications.items():
        # --- THAY ĐỔI Ở ĐÂY: Tạo rule_id dựa trên tên riêng của classification ---
        # Chuẩn hóa tên classification thành dạng kebab-case và thêm tiền tố "wapiti-"
        final_rule_id = f"wapiti-{classification_name.lower().replace(' ', '-')}"

        # Đảm bảo ruleId là duy nhất (thêm hậu tố GUID nếu có trùng lặp)
    ...
    for finding in all_findings:
        ...
        # --- ÁNH XẠ ruleId CHO FINDING (Dựa trên tên riêng) ---
        found_rule_id = default_rule_id 
        type_name = finding.get("_wapiti_type_name", "")
        if type_name:
            # Tạo potential_id_prefix dựa trên tên riêng của classification
            potential_id_prefix = f"wapiti-{type_name.lower().replace(' ', '-')}"
            
            # Tìm rule_obj có id bắt đầu bằng potential_id_prefix
            for r_obj in run_entry["tool"]["driver"]["rules"]:
                if r_obj["id"].startswith(potential_id_prefix):
                    found_rule_id = r_obj["id"]
                    break
        # Không cần cảnh báo ở đây, vì nếu không khớp, nó sẽ tự động dùng default_rule_id
...
if __name__ == "__main__":
    input_file_path = "wapiti.json" 
    output_file_path = "wapiti_sarif.json"

    try:
        with open(input_file_path, 'r', encoding='utf-8') as f:
            wapiti_data = json.load(f)
        
        sarif_converted_data = wapiti_to_sarif_converter(wapiti_data)

        with open(output_file_path, 'w', encoding='utf-8') as f:
            json.dump(sarif_converted_data, f, indent=2, ensure_ascii=False)
        
        print(f"✅ Chuyển đổi thành công từ '{input_file_path}' sang '{output_file_path}'")
        
    except FileNotFoundError:
        print(f"❌ Lỗi: Không tìm thấy tệp đầu vào '{input_file_path}'")
    except json.JSONDecodeError:
        print(f"❌ Lỗi: Không thể phân tích cú pháp tệp JSON '{input_file_path}'. Đảm bảo đó là định dạng JSON hợp lệ.")
